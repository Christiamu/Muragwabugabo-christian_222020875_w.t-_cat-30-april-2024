...
<h1>Librarian Form</h1>
 ...
 <form method="post" onsubmit="return confirmInsert();">
        <label for="LibrarianID">LibrarianID:</label>
        <input type="number" id="LibrarianID" name="LibrarianID"><br><br>

        <label for="FullName">FullName:</label>
        <input type="text" id="UserID" name="UserID" required><br><br>

        <label for="UserName">UserName:</label>
        <input type="text" id="IssueDate" name="IssueDate" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO librarian (LibrarianID, FullName,UserName,Password,Email) VALUES (?,?,?,?,?)");
        $stmt->bind_param("issss", $LibrarianID, $FullName,$UserName,$Password,$Email );

        // Set parameters from POST data with validation (optional)
        $LibrarianID = intval($_POST['LibrarianID']); // Ensure integer for ID
        $FullName = htmlspecialchars($_POST['UserID']); // Prevent XSS
        $UserName = htmlspecialchars($_POST['IssueDate']);
        $Password = htmlspecialchars($_POST['ReturnDate']);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of librarian</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Librarian Form</h2></center>
    <table border="5">
        <tr>
        ...
        // Prepare SQL query to retrieve all librarian
        $sql = "SELECT * FROM librarian";
        $result = $connection->query($sql);

        // Check if there are any librarian
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $appid = $row['LibrarianID']; // Fetch the librarian
                echo "<tr>
                    <td>" . $row['LibrarianID'] . "</td>
                    <td>" . $row['FullName'] . "</td>
                    <td>" . $row['UserName'] . "</td>
                    <td>" . $row['Password'] . "</td>
                    <td>" . $row['Email'] . "</td>
                    <td><a style='padding:4px' href='delete_librarian.php?LibrarianID=$appid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_librarian.php?LibrarianID=$appid'>Update</a></td> 
                </tr>";
            }
        } else {
