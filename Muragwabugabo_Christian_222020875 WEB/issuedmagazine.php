...
<h1>Issuedmagazine</h1>
 ...
 <form method="post" onsubmit="return confirmInsert();">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO issuedmagazine (MagID, UserID,IssueDate,ReturnDate) VALUES (?,?,?,?)");
        $stmt->bind_param("isss", $MagID,$UserID ,$FirstName,$LastName);

        // Set parameters from POST data with validation (optional)
        $MagID = intval($_POST['MagID']); // Ensure integer for ID
        $UserID = htmlspecialchars($_POST['UserID']); // Prevent XSS
...
// SQL query to fetch data from issuedmagazine table
$sql = "SELECT * FROM issuedmagazine";
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of issuedmagazine</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Issuedmagazine</h2></center>
    <table border="5">
        <tr>
        ...
        // Prepare SQL query to retrieve all issuedmagazine
        $sql = "SELECT * FROM issuedmagazine";
        $result = $connection->query($sql);

        // Check if there are any issuedmagazine
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $appid = $row['MagID']; // Fetch the V
                echo "<tr>
                    <td>" . $row['MagID'] . "</td>
                    <td>" . $row['UserID'] . "</td>
                    <td>" . $row['IssueDate'] . "</td>
                    <td>" . $row['ReturnDate'] . "</td>
                    <td><a style='padding:4px' href='delete_issuedmagazine.php?MagID=$appid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_issuedmagazine.php?MagID=$appid'>Update</a></td> 
                </tr>";
            }
        } else {
