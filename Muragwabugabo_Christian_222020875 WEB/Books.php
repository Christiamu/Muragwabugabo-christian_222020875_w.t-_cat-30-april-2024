...
<section>
  <h1>Books Form</h1>
     ...
     <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="number" id="BkID" name="BkID"><br><br>

        <label for="BookName">BookName:</label>
        <input type="text" id="BkNme" name="BkNme" required><br><br>

        <label for="Genre">Genre:</label>
        <input type="text" id="GN" name="GN" required><br><br>
        ...
        <label for="Shelfr">Shelf:</label>
        <input type="text" id="shl" name="shl" required><br><br>
        

        <label for="Row">Row:</label>
        <input type="number" id="rw" name="rw" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO books (BookID, BookName, Genre, Author, Publisher, Shelf, Row) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssss", $BookID, $BookName, $Genre, $Author, $Publisher, $Shelf, $Row);

        // Set parameters from POST data with validation (optional)
        $BookID = intval($_POST['BkID']); // Ensure integer for ID
        $BookName = htmlspecialchars($_POST['BkNme']); // Prevent XSS
        $Genre = htmlspecialchars($_POST['GN']); // Prevent XSS
        $Author = filter_var($_POST['Author'], FILTER_SANITIZE_EMAIL); 
        $Publisher = filter_var($_POST['phn'], FILTER_SANITIZE_NUMBER_INT); 
        $Shelf = htmlspecialchars($_POST['shl']); 
        $Row = intval($_POST['rw']); 
...
// SQL query to fetch data from books table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of books</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <center><h2>Table of Books</h2></center>
    <table border="5">
        <tr>
            <th>BookID</th>
            <th>BookName</th>
        ...
        // Prepare SQL query to retrieve all books
        $sql = "SELECT * FROM books";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $appid = $row['BookID']; // Fetch the BookID
                echo "<tr>
                    <td>" . $row['BookID'] . "</td>
                    <td>" . $row['BookName'] . "</td>
                    <td>" . $row['Genre'] . "</td>
                    ...
                    <td>" . $row['Shelf'] . "</td>
                    <td>" . $row['Row'] . "</td>
                    <td><a style='padding:4px' href='delete_books.php?BookID=$appid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_books.php?BookID=$appid'>Update</a></td> 
                </tr>";
            }
        } else {
