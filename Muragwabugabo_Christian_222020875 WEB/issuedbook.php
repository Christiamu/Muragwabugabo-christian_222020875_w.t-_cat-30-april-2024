...
<h1>Issuedbook</h1>
 ...
 <form method="post" onsubmit="return confirmInsert();">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO issuedbook (BookID, UserID,IssueDate,ReturnDate) VALUES (?,?,?,?)");
        $stmt->bind_param("isss", $BookID, $FirstName,$LastName);

        // Set parameters from POST data with validation (optional)
        $BookID = intval($_POST['BookID']); // Ensure integer for ID
...
// SQL query to fetch data from issuedbook table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of issuedbook</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Issuedbook</h2></center>
    <table border="5">
        <tr>
        ...
        // Prepare SQL query to retrieve all issuedbook
        $sql = "SELECT * FROM issuedbook";
        $result = $connection->query($sql);

        // Check if there are any issuedbook
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $appid = $row['UserID']; // Fetch the issuedbook
                echo "<tr>
                    <td>" . $row['BookID'] . "</td>
                    <td>" . $row['UserID'] . "</td>
                    <td>" . $row['IssueDate'] . "</td>
                    <td>" . $row['ReturnDate'] . "</td>
                    <td><a style='padding:4px' href='delete_issuedbook.php?BookID=$appid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_issuedbook.php?BookID=$appid'>Update</a></td> 
                </tr>";
            }
        } else {
