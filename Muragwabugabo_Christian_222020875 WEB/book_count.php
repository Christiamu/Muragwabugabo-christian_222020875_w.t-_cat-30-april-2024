...
<form method="post" onsubmit="return confirmInsert();">
        <label for="BookNo">BookNo:</label>
        <input type="number" id="BookNo" name="BookNo"><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stmt = $connection->prepare("INSERT INTO book_count (BookNo, UserID) VALUES (?, ?)");
        $stmt->bind_param("is", $BookNo, $UserID);

        // Set parameters from POST data with validation (optional)
        $BookNo = intval($_POST['BookNo']); // Ensure integer for ID
...
// SQL query to fetch data from book_count table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of book_count</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of Book_count</h2></center>
    <table border="5">
        <tr>
            <th>BookNo</th>
            <th>UserID</th>
        ...
        // Prepare SQL query to retrieve all book_count
        $sql = "SELECT * FROM book_count";
        $result = $connection->query($sql);

        // Check if there are any book_count
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $appid = $row['BookNo']; // Fetch the BookNo
                echo "<tr>
                
                    <td>" . $row['BookNo'] . "</td>
                    <td>" . $row['UserID'] . "</td>
                    <td><a style='padding:4px' href='delete_book_count.php?BookNo=$appid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_book_count.php?BookNo=$appid'>Update</a></td> 
                </tr>";
            }
        } else {
